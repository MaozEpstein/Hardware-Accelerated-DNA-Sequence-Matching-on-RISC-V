...
/// Icache debug
   ...
   // fetch control
   el2_ifu_ifc_ctl/*Alex Grinshpun #(.pt(pt)) */ifc (.*
                    );

   // branch predictor
   if (pt.BTB_ENABLE==1) begin  : bpred
      el2_ifu_bp_ctl /* Alex Grinshpun  #(.pt(pt))*/  bp (.*);
   end
   else begin : bpred
...
`endif
 ...
 logic                       ifc_region_acc_fault_bf;       // Access fault. in ICCM region but offset is outside defined ICCM.
   ...
   el2_ifu_aln_ctl/* Alex Grinshpun #(.pt(pt)) */aln (
                                    .*
                                    );


   // icache
   el2_ifu_mem_ctl/*Alex Grinshpun #(.pt(pt)) */mem_ctl
     (.*,
