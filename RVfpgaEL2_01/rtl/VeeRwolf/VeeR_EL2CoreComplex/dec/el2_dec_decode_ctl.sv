...
import el2_pkg::*;
#(
//Alex Grinshpun `include "el2_param.svh"
 )
  (
...
`endif
 ...
 ibex_decoder ibex_decoder  (
	...
	.illegal_insn_o	(),        // illegal instr encountered
	.ebrk_insn_o	(),           // trap instr encountered
	.mret_insn_o	(),           // return from exception instr
							// encountered
	.dret_insn_o	(),           // return from debug instr encountered
	.ecall_insn_o	(),          // syscall instr encountered
	.wfi_insn_o		(),            // wait for interrupt instr encountered
	.jump_set_o		(),            // jump taken set signal
	.branch_taken_i	(),        // registered branch decision
	.icache_inval_o	(),

  // from IF-ID pipeline register
	.instr_first_cycle_i	('0),   // instruction read is in its first cycle
	.instr_rdata_i			(dec_i0_instr_d),         // instruction read from memory/cache
  //input  logic [31:0]          instr_rdata_alu_i,     // instruction read from memory/cache
                                                      // replicated to ease fan-out)

	.illegal_c_insn_i		('0),      // compressed instruction decode failed

	.imm_a_mux_sel_o		(),       // immediate selection for operand a
	.imm_b_mux_sel_o		(),       // immediate selection for operand b
	.bt_a_mux_sel_o			(),        // branch target selection operand a
	.bt_b_mux_sel_o			(),        // branch target selection operand b
	.imm_i_type_o			(),
	.imm_s_type_o			(),
	.imm_b_type_o			(),
	.imm_u_type_o			(),
	...
	.rf_wdata_sel_o			(),   // RF write data selection
	.rf_we_o				(),          // write enable for regfile
	.rf_raddr_a_o			(),
	.rf_raddr_b_o			(),
	.rf_waddr_o				(),
	.rf_ren_a_o				(),          // Instruction reads from RF addr A
	.rf_ren_b_o				(),          // Instruction reads from RF addr B
	
	
	.alu_operator_o			(),        // ALU operation selection
	.alu_op_a_mux_sel_o		(),    // operand a selection: reg value, PC,
						// immediate or zero
	.alu_op_b_mux_sel_o		(),    // operand b selection: reg value or
						// immediate
	.alu_multicycle_o		(),      // ternary bitmanip instruction
	
	.mult_en_o				(),             // perform integer multiplication
	.div_en_o				(),              // perform integer division or remainder
	.mult_sel_o				(),            // as above but static, for data muxes
	.div_sel_o				(),             // as above but static, for data muxes
	.multdiv_operator_o		(),
	.multdiv_signed_mode_o	(),
	
	
	.csr_access_o			(),          // access to CSR
	.csr_op_o				(),              // operation to perform on CSR
	
	.data_req_o				(),            // start transaction to data memory
	.data_we_o				(),             // write enable
	.data_type_o			(),           // size of transaction: byte, half
						// word or word
	.data_sign_extension_o	(), // sign extension for data read from
						// memory
	
	.jump_in_dec_o			(),         // jump is being calculated in ALU
	.branch_in_dec_o		()
);
`endif
/*   
   fancy_instructions fancy_instructions
  (
		.dec_i0_rs1_en_d		(dec_i0_rs1_en_d	),               // rs1 enable at decode
		.dec_i0_rs2_en_d		(dec_i0_rs2_en_d	),               // rs2 enable at decode
		                         
		.dec_i0_rs1_d			(dec_i0_rs1_d		),                  // rs1 logical source
		.dec_i0_rs2_d			(dec_i0_rs2_d		),                  // rs2 logical source
		                         
		.dec_i0_immed_d			(dec_i0_immed_d		),                // 32b immediate data decode
		                         
		                         
		.dec_i0_br_immed_d		(dec_i0_br_immed_d	),             // 12b branch immediate
		                         
		.i0_ap					(i0_ap				),                       // alu packets
		                         
		.dec_i0_decode_d		(dec_i0_decode_d	),               // i0 decode
		                         
		.dec_i0_alu_decode_d	(dec_i0_alu_decode_d),           // decode to D-stage alu
		.dec_i0_branch_d		(dec_i0_branch_d	),               // Branch in D-stage
		.scan_mode				(scan_mode			)
   );
   */
