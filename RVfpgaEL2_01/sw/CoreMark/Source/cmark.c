...
MAIN_RETURN_TYPE main_cmark(int argc, char *argv[]) {
        ...
        if (sizeof(struct list_head_s)>128) {
                printfNexys("list_head structure too big for comparable data!\n");
...
//        stop_time();
        ...
        switch (seedcrc) { /* test known output for common seeds */
                case 0x8a02: /* seed1=0, seed2=0, seed3=0x66, size 2000 per algorithm */
                        known_id=0;
                        printfNexys("6k performance run parameters for coremark.\n");
                        break;
                case 0x7b05: /*  seed1=0x3415, seed2=0x3415, seed3=0x66, size 2000 per algorithm */
                        known_id=1;
                        printfNexys("6k validation run parameters for coremark.\n");
                        break;
                case 0x4eaf: /* seed1=0x8, seed2=0x8, seed3=0x8, size 400 per algorithm */
                        known_id=2;
                        printfNexys("Profile generation run parameters for coremark.\n");
                        break;
                case 0xe9f5: /* seed1=0, seed2=0, seed3=0x66, size 666 per algorithm */
                        known_id=3;
                        printfNexys("2K performance run parameters for coremark.\n");
                        break;
                case 0x18f2: /*  seed1=0x3415, seed2=0x3415, seed3=0x66, size 666 per algorithm */
                        known_id=4;
                        printfNexys("2K validation run parameters for coremark.\n");
                        break;
                default:
                        total_errors=-1;
                        break;
        ...
        if (known_id>=0) {
                for (i=0 ; i<default_num_contexts; i++) {
                        ...
                        if ((results[i].execs & ID_LIST) &&
                                ...
                                printfNexys("[%u]ERROR! list crc 0x%04x - should be 0x%04x\n",i,results[i].crclist,list_known_crc[known_id]);
                                results[i].err++;
                        }
                        if ((results[i].execs & ID_MATRIX) &&
                                (results[i].crcmatrix!=matrix_known_crc[known_id])) {
                                printfNexys("[%u]ERROR! matrix crc 0x%04x - should be 0x%04x\n",i,results[i].crcmatrix,matrix_known_crc[known_id]);
                                results[i].err++;
                        }
                        if ((results[i].execs & ID_STATE) &&
                                (results[i].crcstate!=state_known_crc[known_id])) {
                                printfNexys("[%u]ERROR! state crc 0x%04x - should be 0x%04x\n",i,results[i].crcstate,state_known_crc[known_id]);
                                results[i].err++;
                        }
...
//        ee_printf("Memory location  : %s\n",MEM_LOCATION);
        ...
        printfNexys("seedcrc          : 0x%04x\n",seedcrc);
        if (results[0].execs & ID_LIST)
                for (i=0 ; i<default_num_contexts; i++)
                        printfNexys("[%d]crclist       : 0x%04x\n",i,results[i].crclist);
        if (results[0].execs & ID_MATRIX)
                for (i=0 ; i<default_num_contexts; i++)
                        printfNexys("[%d]crcmatrix     : 0x%04x\n",i,results[i].crcmatrix);
        if (results[0].execs & ID_STATE)
                for (i=0 ; i<default_num_contexts; i++)
                        printfNexys("[%d]crcstate      : 0x%04x\n",i,results[i].crcstate);
        for (i=0 ; i<default_num_contexts; i++)
                printfNexys("[%d]crcfinal      : 0x%04x\n",i,results[i].crc);
        if (total_errors==0) {
                printfNexys("Correct operation validated. See readme.txt for run and reporting rules.\n");
#if HAS_FLOAT
                if (known_id==3) {
        ...
        if (total_errors>0)
                printfNexys("Errors detected\n");
        if (total_errors<0)
                printfNexys("Cannot validate operation for these seed values, please compare with results on a known platform.\n");
...
void printmat(MATDAT *A, ee_u32 N, char *name) {
        ee_u32 i,j;
        printfNexys("Matrix %s [%dx%d]:\n",name,N,N);
        for (i=0; i<N; i++) {
                for (j=0; j<N; j++) {
                        if (j!=0)
                                printfNexys(",");
                        printfNexys("%d",A[i*N+j]);
                }
                printfNexys("\n");
        }
}
...
ee_u8 check_data_types() {
        ee_u8 retval=0;
        if (sizeof(ee_u8) != 1) {
                printfNexys("ERROR: ee_u8 is not an 8b datatype!\n");
                retval++;
        }
        if (sizeof(ee_u16) != 2) {
                printfNexys("ERROR: ee_u16 is not a 16b datatype!\n");
                retval++;
        }
        if (sizeof(ee_s16) != 2) {
                printfNexys("ERROR: ee_s16 is not a 16b datatype!\n");
                retval++;
        }
        if (sizeof(ee_s32) != 4) {
                printfNexys("ERROR: ee_s32 is not a 32b datatype!\n");
                retval++;
        }
        if (sizeof(ee_u32) != 4) {
                printfNexys("ERROR: ee_u32 is not a 32b datatype!\n");
                retval++;
        }
        if (sizeof(ee_ptr_int) != sizeof(int *)) {
                printfNexys("ERROR: ee_ptr_int is not a datatype that holds an int pointer!\n");
                retval++;
        }
        if (retval>0) {
                printfNexys("ERROR: Please modify the datatypes in core_portme.h!\n");
        }
        return retval;
}
/*
...
void portable_init(core_portable *p, int *argc, char *argv[])
{
        if (sizeof(ee_ptr_int) != sizeof(ee_u8 *)) {
                printfNexys("ERROR! Please define ee_ptr_int to a type that holds a pointer!\n");
        }
        if (sizeof(ee_u32) != 4) {
                printfNexys("ERROR! Please define ee_u32 to a 32b unsigned type!\n");
        }
        p->portable_id=1;
}
